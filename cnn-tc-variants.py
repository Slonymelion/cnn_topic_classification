...
def TopicCollate(data):
    ...
    #X = [w[0]+[PAD]*(meanlen-len(w[0])) for w in data]
    #X = [x[:meanlen] for x in X]
...
train = [x for x in train if x[1]<=12]
...
params['lr'] = 1e-4
params['note'] = 'cut the training labels down to 12'
anneal_patience = 2  # for learning rate annealing
early_stop_count = 2  # for early stopping
num_workers = 6
run_test = True
